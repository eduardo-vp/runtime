...
/* static */ void UnwindInfoTable::PublishUnwindInfoForMethod(TADDR baseAddress, PT_RUNTIME_FUNCTION methodUnwindData, int methodUnwindDataCount)
{
    ...
    if (unwindInfo == NULL)
    {
        // Speculatively create the table for this RangeSection and try to install it
        // via CAS. The losing thread (if any) deletes its speculative table. This avoids
        // a process-wide lock on the cold path - per-RangeSection init is rare, but a
        // shared lock here would serialize ALL first-time publishes across all sections.
        ...
        {
            // Another thread won the race. Our candidate will be destroyed by the
            // NewHolder, which also tears down the OS registration via ~UnwindInfoTable.
