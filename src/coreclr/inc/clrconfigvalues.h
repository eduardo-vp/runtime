...
/// Async Stack Traces
///
RETAIL_CONFIG_DWORD_INFO(EXTERNAL_HideAsyncDispatchFrames, W("HideAsyncDispatchFrames"), 1, "Controls async stack trace behavior in Environment.StackTrace: 0 = show all frames with async continuation stitching, 1 (default) = hide non-async frames below the first runtime async frame, 2 = keep non-async frames between runtime async frames but truncate trailing ones, 3 = disable async stitching entirely and show only physical call stack frames.")
